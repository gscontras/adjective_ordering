...
o = read.csv("~/Documents/git/cocolab/adjective_ordering/experiments/analysis/class-naturalness.csv",header=T)
o$expt = "preference"
head(o)
#o <- subset(o,select=c("class1","response","bootsci_high","bootsci_low","expt"))
colnames(o) <- c("X","class","N","average","YMin","YMax","expt")
# faultless data 
...
colnames(f) <- c("X","class","N","average","YMin","YMax","expt")
# subjectivity data 
...
colnames(s) <- c("X","class","N","average","YMin","YMax","expt")
# corpus
c = read.csv("~/Documents/git/cocolab/adjective_ordering/experiments/analysis/corpus_class_averages.csv",header=T)
c$expt = "corpus"
head(c)
colnames(c) <- c("X","class","N","average","YMin","YMax","expt")
...
d$expt = factor(d$expt,levels=c("preference","corpus","subjectivity","faultless"))

#write.csv(d,"~/Documents/git/cocolab/adjective_ordering/experiments/analysis/results-by-class.csv")
