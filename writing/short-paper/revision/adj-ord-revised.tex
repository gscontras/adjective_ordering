...
A nested linear model comparison confirmed that
noun-specific subjectivity did not explain any variance in ordering preference above and beyond adjective-level subjectivity ($F(1,255) = 0.44$, $p < 0.51$). Thus, we fail to find evidence of noun-specific effects.
...
In our investigation, we established two empirical constructs: the preferences themselves, which we measured using naturalness ratings and validated with corpus counts; and adjective subjectivity, which we operationalized as potential for faultless disagreement and corroborated with a direct ``subjectivity'' measure. Our findings suggest a clear path forward: an adjective's semantics can indeed predict its distance from the modified noun, such that less subjective adjectives occur linearly closer to nouns they modify. This preference is not deterministic; non-preferred orderings of adjectives can serve a communicative purpose, for example contrastiveness is discourse \cite{martin1969determinants,martin1970juncture,hill1960,vendler1963}. This constrastiveness follows straightforwardly from a markedness implicature \cite{grice}: marked forms (i.e., non-preferred orderings of adjectives) yield marked interpretations (i.e., atypical modification constituency). The work lies in determining the preferred orderings from which  contrastive uses depart, which has been the focus of our study.
...
Our findings have implications for studies of word order both within and beyond the nominal domain. First, looking at other classes of nominal modifiers, researchers have observed that demonstratives (e.g., \emph{this} and \emph{that}), numerals, and adjectives 
Universal 20 \cite{greenberg1963,culbertsonadger2013}.

Second, adverb ordering.
