...
%
On the basis of $\epsilon_{adj}$, each adjective has some probability $p_{adj}(\texttt{ref})$ of correctly classifying the intended referent(s) \texttt{ref}: 

\setcounter{equation}{2}
\begin{equation}
p_{adj}(\texttt{ref}) = 1-\epsilon_{adj}
\end{equation}

If we assume a fixed misclassification potential $\epsilon$ for each adjective and a truly intersective semantics for modification, the probability of arriving at the correct referent once a noun has been modified by multiple adjectives does not depend on the order of the adjectives (i.e., the order of semantic composition). %To see why, consider the process of reference resolution from the perspective of the intended referent. 
...
We break the commutativity of noise and thus the order-independence of modification once we recognize that $\epsilon_{adj}$ is not a fixed value, but rather varies with the size of the set to be classified, $|\textrm{NP}|$ (i.e., the number of objects under consideration).\footnote{For convenience, we identify NP with its extension, a set of objects.} With a fixed processing budget, ​more classifications to be made will necessarily mean making each with fewer resources; the precision of a stochastic classification is monotonically related to processing resources. Thus, as $|\textrm{NP}|$ increases, the precision of each individual classification decreases and so the potential for misclassification grows. We model this tendency by revising $p_{adj}(\texttt{ref})$ so that $\epsilon_{adj}$ depends on the size of the NP denotation that $adj$ restricts, with the constraint that $\epsilon_{adj}(|\textrm{NP}|) \leq \epsilon_{adj}(|\textrm{NP}|+1)$:
...
In a case with two adjectives, the probability that the full multi-adjective NP correctly classifies the intended referent \texttt{ref}, $p_{N\!P}(\texttt{ref})$, takes into account the probability that $adj_1$ correctly classifies \texttt{ref} in NP$_1$, $p_{adj_1}(\texttt{ref},\textrm{NP}_2)$, and that $adj_2$ correctly classifies \texttt{ref} in NP$_2$, $p_{adj_2}(\texttt{ref},\textrm{NP}_2)$. However, $p_{adj_2}(\texttt{ref},\textrm{NP}_2)$ depends on the size of NP$_2$, which itself depends on potential classification errors from $adj_1$. We must therefore consider all possible values for NP$_2$.

To calculate the probability of each possible NP$_2$, the function $F_{i}$ in \Next looks up the probability that $adj_i$ (in)correctly classifies each potential element $x$; $V(x,adj_i)$ serves as our ground truth, returning \texttt{true} just in case $x$ actually holds the property named by $adj_i$. The first outcome for $F_i$ in \Next corresponds to the probability of correctly including an element in NP$_2$ that holds the property named by $adj_i$; the second outcome corresponds to the probability of correctly excluding elements that do not hold the relevant property. The final two outcomes correspond to the probabilities associated with misclassifications.

\begin{equation}
F_i(x,\textrm{NP}_2,\textrm{NP}_1) = \left\{\begin{array} {l}
1 - \epsilon_i(|\textrm{NP}_1|) \textrm{ if } x \in \textrm{NP}_2\ \&\ V(x,adj_i)  \\
1 - \epsilon_i(|\textrm{NP}_1|) \textrm{ if } x \notin \textrm{NP}_2\ \&\ \neg V(x,adj_i) \\
\epsilon_i(|\textrm{NP}_1|) \textrm{ if } x \notin \textrm{NP}_2\ \&\ V(x,adj_i) \\
\epsilon_i(|\textrm{NP}_1|) \textrm{ if } x \in \textrm{NP}_2\ \&\ \neg V(x,adj_i)
\end{array}
\right. 
\end{equation}

To calculate the probability of successful referent classification at the level of NP, $p_{N\!P}(\texttt{ref})$,
%Because the size of NP$_2$ can vary depending on errors made when $adj_1$ restricts NP$_1$, 
we sum over possible values of NP$_2$, assuming that \texttt{ref} was correctly classified by $adj_1$ and so made it into the NP$_2$ denotation (i.e., $\texttt{ref} \in \textrm{NP}_2$). For each potential NP$_2$, we find the probability of successful classification by $adj_2$, $p_{adj_2}(\texttt{ref}, \textrm{NP}_2)$, and multiply it by the probability of arriving at that NP$_2$:

\begin{equation}
\label{NP-prob}
p_{N\!P}(\texttt{ref}) = \sum_{\substack{\textrm{NP}_2\subseteq\textrm{NP}_1\\ \textrm{where}\ \texttt{ref}\in\textrm{NP}_2}} p_{adj_2}(\texttt{ref}, \textrm{NP}_2) \cdot \prod_{x \in \textrm{NP}_1} F_{adj_1}(x, \textrm{NP}_2, \textrm{NP}_1)  
\end{equation}
%
...
To see the role of subjectivity-based ordering in maximizing the probability of successful reference resolution, consider the choice between \emph{small brown box} vs.~\emph{brown small box} in \ref{default} vs.~\ref{dispreferred}.\\ 
...
\end{minipage}
\begin{minipage}{.5\textwidth}
	\ex.[(8)] \label{dispreferred}
	\Tree [.NP [.AP \emph{brown} ] [.NP$_2$ [.AP \emph{small} ] [.NP$_1$ \emph{box} ] ] ]
	
\end{minipage}\\

\noindent Suppose there are three boxes: $box_{s}$ is small but not brown, $box_{b}$ is brown but not small, and $box_{sb}$ is both small and brown. To calculate the probability of successfully resolving the referent to the small brown box for the two adjective orderings, we will need each adjective's potential for misclassification; we use the subjectivity scores from \cite{scontrasetal2017adjectives} to set the upper limit of these values (i.e., when $|\textrm{NP}|=3$), %\gcs{more about our choices} 
and assume that $\epsilon$ decreases by 0.04 with each decrease in $|\textrm{NP}|$.\footnote{
%\gcs{even more of a disclaimer} 
These values are adopted for illustrative purposes only. Subjectivity scores are likely an inflated estimate of an adjective's potential for misclassification $\epsilon$, and the decrease of 0.04 for decreasing $|\textrm{NP}|$ is chosen arbitrarily. Still, any values for $\epsilon_{small}(|\textrm{NP}|)$ and $\epsilon_{brown}(|\textrm{NP}|)$ would do so long as $\epsilon_{small}(|\textrm{NP}|) > \epsilon_{brown}(|\textrm{NP}|)$ for any $|\textrm{NP}|$.} We adopt the following values:
...
% \multicolumn{3}{c|}{$F_{adj_1}(x, \textrm{NP}_2, \textrm{NP}_1)$} & & $p_{adj_2}(\texttt{ref}, \textrm{NP}_2)$ \\ \hline 
	...
	T (0.80) & F (0.20) & F (0.80) & 0.44 & 0.128 \\ \hline
	\multicolumn{4}{r}{\textbf{$p_{{small\ brown\ box}}(\texttt{ref}) = $}}\hspace{-10pt} & \textbf{0.32} \\ 
	\end{tabular}
	\end{equation}
	

In (\ref{dispreferred-calculation}), we perform the same calculations for the dispreferred ordering in \ref{dispreferred}; with this ordering, the probability of correctly classifying \texttt{ref} for the full NP is 0.30.
...
T (0.36) & F (0.36) & F (0.64) & 0.88 & 0.083 \\ \hline
...
As the calculations above demonstrate, ordering with respect to decreasing subjectivity results in a higher probability of successfully classifying the intended referent than the reverse order. By ensuring that $|\textrm{NP}_2| \leq |\textrm{NP}_1|$ in the case of restrictive adjectival modification, this pattern holds generally; this fact is summarized by the following theorem:

\ex.[(12)] If $\epsilon_{adj}(i)$ is monotonic with respect to $i$, and $\epsilon_{adj_1}(i) < \epsilon_{adj_2}(i)$ for all $i$, then 
...
Subjectivity-based ordering preferences maximize the probability of successful reference resolution in cases of resource bounded computation. 
