...
\Tree [.NP$_4$ [.AP \emph{small} ] [.NP$_3$ [.AP \emph{brown} ] [.NP$_2$ [.AP \emph{cardboard} ] [.NP$_1$ \emph{box} ] ] ] ] 
...
%
On the basis of $\epsilon_{adj}$, each adjective has some probability $p_{adj}(\texttt{ref})$ of correctly classifying the intended referent(s): 
...
Given the commutativity of noise in intersective modification, a nominal with two adjectives will correctly classify the intended referent with probability $p_{adj_1}(\texttt{ref}) \cdot p_{adj_n}(\texttt{ref})$. In \emph{small brown box}, the probability that the intended referent will remain in the full nominal denotation is equal to $p_{small}(\texttt{ref}) \cdot p_{brown}(\texttt{ref})$, irrespective of order. If the pressure for subjectivity-based ordering preferences %ultimately arises not out of an avoidance of any misclassifications, but 
...
We break the commutativity of noise and thus the order-independence of modification once we recognize that $\epsilon_{adj}$ is not a fixed value, but rather varies with the size of the set to be classified $|\textrm{NP}|$ (i.e., the number of objects under consideration). With a fixed processing budget, ​more classifications to be made will necessarily mean making each with fewer resources; the precision of a stochastic classification is monotonically related to processing resources. Thus, as $|\textrm{NP}|$ increases, the precision of each individual classification decreases and so the potential for misclassification grows. We model this tendency by revising $p_{adj}(\texttt{ref})$ so that $\epsilon_{adj}$ depends on the size of the NP denotation that $adj$ restricts, with the constraint that $\epsilon_{adj}(|\textrm{NP}|) \leq \epsilon_{adj}(|\textrm{NP}|+1)$:
...
\gcs{a walk-through of the math} The probability that the full multi-adjective NP correctly classifies the intended referent \texttt{ref} so that \texttt{ref} makes it into the NP denotation, $p_{N\!P}(\texttt{ref})$, takes into account the probability that $adj_1$ correctly classifies \texttt{ref} in NP$_1$ and that $adj_2$ correctly classifies \texttt{ref} in NP$_2$; because the size of NP$_2$ can vary depending on errors made when $adj_1$ restricts NP$_1$, we sum over possible values $i$ of $|\textrm{NP}_2|$, assuming that \texttt{ref} was correctly classified by $adj_1$ and so made it into the NP$_2$ denotation (i.e., $\texttt{ref} \in \textrm{NP}_2$):\footnote{The precise value of $|\textrm{NP}_2|$ depends on $p_{adj_1}(\texttt{ref}, \textrm{NP}_1)$. $|\textrm{NP}_2|$ is a random variable whose value depends on two Binomial distributions, which simulate the outcome of $n$ weighted coin flips:
	$$|\textrm{NP}_2| =  \textrm{binomial}(1-\epsilon_{adj_1}(|\textrm{NP}_1|),2) + \textrm{binomial}(\epsilon_{adj_1}(|\textrm{NP}_1|),2).$$
Binomial($1 - \epsilon_{adj_1}(|\textrm{NP}_1|),2$) corresponds to the number of correct classifications for $adj_1$, while Binomial($\epsilon_{adj_1}(|\textrm{NP}_1|),2$) corresponds to the number of incorrect classifications for $adj_1$. The number of (in)correct classifications determines the size of NP$_2$.}
...
To see the role of subjectivity-based ordering in maximizing the probability of successful reference resolution, consider the choice between \emph{small brown box} vs.~\emph{brown small box}. Suppose there are four boxes, two of which are small, two brown, and one small and brown. To calculate the probability of successfully resolving the referent for the two adjective orderings, we will need each adjective's potential for misclassification; we use the subjectivity scores from \cite{scontrasetal2017adjectives} to set the upper limit of these values (i.e., when $|\textrm{NP}|=4$), \gcs{more about our choices} and assume that $\epsilon$ decreases by 0.04 with each decrease in $|\textrm{NP}|$.\footnote{\gcs{even more of a disclaimer} These values are adopted for illustrative purposes only. Subjectivity scores are likely an inflated estimate of an adjective's potential for misclassification $\epsilon$, and the decrease of 0.04 for decreasing $|\textrm{NP}|$ is chosen arbitrarily. Still, any values for $\epsilon_{small}(|\textrm{NP}|)$ and $\epsilon_{brown}(|\textrm{NP}|)$ would do so long as $\epsilon_{small}(|\textrm{NP}|) > \epsilon_{brown}(|\textrm{NP}|)$ for any $|\textrm{NP}|$.} We adopt the following values:
\begin{align}
\epsilon_{small}(|\textrm{NP}|=4) = 0.64 & & \epsilon_{brown}(|\textrm{NP}|=4) = 0.20 \nonumber\\
\epsilon_{small}(|\textrm{NP}|=3) = 0.60 & & \epsilon_{brown}(|\textrm{NP}|=3) = 0.16 \nonumber\\
\epsilon_{small}(|\textrm{NP}|=2) = 0.56 & & \epsilon_{brown}(|\textrm{NP}|=2) = 0.12 \nonumber\\
\epsilon_{small}(|\textrm{NP}|=1) = 0.52 & & \epsilon_{brown}(|\textrm{NP}|=1) = 0.08
\end{align}

In the \emph{small brown box} vs.~\emph{brown small box} example, the first adjective to compose will operate over the set of four boxes (i.e., $|\textrm{NP}_1| = 4$). The second adjective will operate over a set that has been restricted by the first adjective, so we ensure that $|\textrm{NP}_2| \leq |\textrm{NP}_1|$. 
%If we suppose that the result of (i.e., $N2 = 2$); Here we consider a case where the first adjective 
As the following calculations demonstrate, ordering with respect to decreasing subjectivity results in a higher probability of successfully classifying the intended referent, (\ref{small-brown-ref}), than the reverse order, (\ref{brown-small-ref}). By ensuring that $|\textrm{NP}_2| \leq |\textrm{NP}_1|$ in the case of restrictive adjectival modification, this pattern holds generally.

\begin{align} \label{small-brown-ref}
p_{small\ brown\ box}(\texttt{ref}) &= \sum_i p_{brown}(\texttt{ref},\textrm{NP}_1) \cdot p_{small}(\texttt{ref},\textrm{NP}_2) \cdot p(|\textrm{NP}_2| = i, \texttt{ref} \in \textrm{NP}_2) \nonumber\\
&= \sum_i (1 - \epsilon_{brown}(4)) \cdot (1 - \epsilon_{small}(|\textrm{NP}_2|)) \cdot p(|\textrm{NP}_2| = i, \texttt{ref} \in \textrm{NP}_2) \nonumber\\
&= \textbf{0.35}
\end{align}

\begin{align} \label{brown-small-ref}
p_{brown\ small\ box}(\texttt{ref}) &= \sum_i p_{small}(\texttt{ref},\textrm{NP}_1) \cdot p_{brown}(\texttt{ref},\textrm{NP}_2) \cdot p(|\textrm{NP}_2| = i,  \texttt{ref} \in \textrm{NP}_2) \nonumber\\
&= \sum_i (1 - \epsilon_{small}(4)) \cdot (1 - \epsilon_{brown}(|\textrm{NP}_2|)) \cdot p(|\textrm{NP}_2| = i,  \texttt{ref} \in \textrm{NP}_2) \nonumber\\
&= \textbf{0.31}
\end{align}
