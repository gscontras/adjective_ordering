...
We break the commutativity of noise and thus the order-independence of modification once we recognize that $\epsilon_{adj}$ is not a fixed value, but rather varies with the size of the set to be classified $|\textrm{NP}|$ (i.e., the number of objects under consideration). With a fixed processing budget, ​more classifications to be made will necessarily mean making each with fewer resources; the precision of a stochastic classification is monotonically related to processing resources. Thus, as $|\textrm{NP}|$ increases, the precision of each individual classification decreases and so the potential for misclassification grows. We model this tendency by revising $p_{adj}(\texttt{ref})$ so that $\epsilon_{adj}$ depends on the size of NP, with the constraint that $\epsilon_{adj}(|\textrm{NP}|) \leq \epsilon_{adj}(|\textrm{NP}|+1)$:
...
The probability that a multi-adjective NP will correct classify the intended referent, $p_{N\!P}(\texttt{ref})$, continues to depend on the probabilities that each individual adjective will correctly classify the intended referent, but now these probabilities depend on the size of the set to be classified:
...
%\end{align}
Adjectives closer to the noun will compose earlier semantically, and so the number $|\textrm{NP}|$ of potential referents they must classify will be larger (cf.~the example in Fig.~\ref{mod-fig}). Because adjectives that compose earlier classify a larger set, we maximize $p_{N\!P}(\texttt{ref})$ by ensuring that adjectives with lower subjectivity (i.e., a lower $\epsilon$) compose earlier.

To see the role of subjectivity-based ordering in maximizing the probability of successful reference resolution, consider the choice between \emph{small brown box} vs.~\emph{brown small box}. Suppose there are four boxes, two of which are small, two brown, and one small and brown. To calculate the probability of successfully resolving the referent for the two adjective orderings, we will need each adjective's potential for misclassification; we use the subjectivity scores from \cite{scontrasetal2017adjectives} to set the upper limit of these values in this toy example (i.e., when $|\textrm{NP}|=4$).\footnote{Subjectivity scores are likely an inflated estimate of an adjective's potential for misclassification $\epsilon$; any values for $\epsilon_{small}(|\textrm{NP}|)$ and $\epsilon_{brown}(|\textrm{NP}|)$ would do so long as $\epsilon_{small}(|\textrm{NP}|) > \epsilon_{brown}(|\textrm{NP}|)$ for any $|\textrm{NP}|$.} For illustrative purposes, we adopt the following values:
\begin{align}
...
In the \emph{small brown box} vs.~\emph{brown small box} example, the first adjective to compose will operate over the set of four boxes (i.e., $|\textrm{NP}_1| = 4$). The second adjective will operate over a set that has been restricted by the first adjective, so we ensure that $|\textrm{NP}_2| \leq |\textrm{NP}_1|$.\footnote{$|\textrm{NP}_2|$ depends on $p_{adj_1}(\texttt{ref}, \textrm{NP}_1)$. $|\textrm{NP}_2|$ is a random variable whose value depends on two Binomial distributions, which simulate the outcome of $n$ weighted coin flips:
