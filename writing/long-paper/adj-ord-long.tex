...
Participants demonstrated little preference for adjective order when the descriptions were nonsense. For this reason, we excluded responses to nonsensical descriptions from the analyses of subjectivity below. \ndg{how many excluded in this way?} Fig.~\ref{fig:exp3-results} (\emph{preference}) plots mean naturalness scores by adjective class for the descriptions that made sense to participants. Greater values signal that a class's adjectives are preferred in first position, farther from the modified noun; values around 0.5 indicate that neither order is preferred. 
...
\begin{figure}
	...
	\caption{Mean naturalness ratings plotted against mean subjectivity scores for each of the 78 adjectives tested in Expt.~3. Superlatives and outlier adjectives are labeled in red. \ndg{how about putting the -est in green and the other outliers in blue or some other color?}}\label{fig:exp3-nat-subj}
\end{figure}

\begin{figure}
	\centering\includegraphics[width=3.5in]{plots/naturalness-subjectivity_class-difference_no-sup.eps}
	\caption{Mean configuration naturalness ratings plotted against subjectivity difference scores for each pair of adjective classes tested in Expt.~3. \ndg{why is this so pixelated?}}\label{fig:exp3-nat-subj-diff}
\end{figure}
...
We also looked at the contribution of frequency and length in predicting ordering preferences. Treating subjectivity, frequency, and length as predictors in a linear regression predicting naturalness ratings (excluding superlatives), the model accounts for 70\% of the variance ($r^{2}=0.70$). \ndg{and the extended model explains more variance? ie are freq and length predictive beyond subj? and vice versa?} If we remove outlier adjectives that fall more than three standard deviations away from the predicted value of the model (there were six: \emph{mini}, \emph{frozen}, \emph{solid}, \emph{current}, \emph{daily}, \emph{designated}), the model performs better, accounting for 76\% of the variance ($r^{2}=0.76)$.
